...
const Header = () => {
    ...
    const {navigate, user,isOwner,setShowAgencyReg,properties,searchQuery,setSearchQuery,searchedCities, setSearchedCities} = useContext(AppContext);
              ...
              <div className={`${menuOpened ? "fixed top-15 right-12 bg-gray-50 z-50 text-black px-12 py-6 shadow-2xl rounded-sm w-52": ""}hidden lg:flex ml-24 flex-1 justify-center`}>
               ...
               <div className='flex flex-row items-center gap-3 md:gap-5'>
                    ...
                    <div className={`${showSearch && "bg-gray-200"} hidden md:flex flex-row justify-center items-center gap-1 px-2 py-0.5  rounded-full`}>
                       <input value={searchQuery} onChange={(e)=>handleSearchChange(e)} type="text" className={`${showSearch ? "opacity-100 w-50 px-2" : "opacity-0 w-0 px-0"} transition-all duration-200 outline-none  h-full text-black text-sm`} placeholder='type here..' />
